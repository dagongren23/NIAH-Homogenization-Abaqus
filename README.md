...
reference_outputs/structural_data/  Structure-level data
reproducibility/                    Run guide and execution protocol
```
...
Do not copy only selected `.py` files: the plug-in depends on the complete `niah_core` directory.
...
3. Select the listed analysis type, preprocessing mode, primary element type, and a mesh tolerance of `1.0E-3`. The supplied reference runs used eight solver CPUs; for new runs, another even CPU count may be used provided that it does not exceed the computer's available CPU count.
...
See [REPRODUCIBILITY.md](reproducibility/REPRODUCIBILITY.md) for model groups, execution details, and release-validation status.
